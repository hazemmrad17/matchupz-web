...
public function index(Request $request, PaginatorInterface $paginator, EntityManagerInterface $entityManager): Response
{
    ...
    foreach ($allContracts as $contract) {
        ...
        if ($dateFin && $dateFin >= $currentDate && $dateFin <= $thresholdDate) {
            $expiringContractIds[] = $contract->getIdContrat();
            ...
            $phoneNumber = $this->getParameter('twilio_to_number');
            ...
            // Message body
            $body = "Alert: Contract #$contratId titlé '$titre' avec le sponsor '$sponsorNom' du $dateDebut jusqu'à $dateFinFormatted, montant " . number_format($montant, 2) . ", expire bientôt le $dateFinFormatted. Veuillez réviser.";
            ...
            try {
                ...
                sleep(2); // Allow time for status update
                $messageDetails = $twilio->messages($message->sid)->fetch();

                if (in_array($messageDetails->status, ['delivered', 'sent'])) {
                    ...
                    $this->logger->warning('SMS not delivered for contract ID ' . $contratId, [
                        'sid' => $message->sid,
                        'status' => $messageDetails->status,
                        'error_code' => $messageDetails->errorCode,
                        'error_message' => $messageDetails->errorMessage,
                    ]);
                    $smsResults[] = [
                        'contractId' => $contratId,
                        'status' => 'error',
                        'message' => 'SMS not delivered: ' . $messageDetails->status,
                    ];
                }
    ...
    $errorCount = count($smsResults) - $successCount;
