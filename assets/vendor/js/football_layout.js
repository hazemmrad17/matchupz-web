...
const events = {
    attachAll: function() {
        ...
        $players.on('click', function(e) {
            e.preventDefault();
            const $el = $(this);
            if ($el.hasClass('empty') || $('.js-player.active').length) return false;
    ...
    attachClose: function() {
        $(document).on('click.closePlayer', function(e) {
            ...
            if ($target.closest('.js-player.active').length || $target.closest('.player__card').length) {
                return;
            }
            scenes.unfocusPlayer();
            $(document).off('click.closePlayer');
        });
    }
};
...
const scenes = {
    preLoad: function() {
        ...
        $terrain.css('background', 'url(/path/to/field-image.jpg) no-repeat center/cover');
    },
    ...
    startLoading: function() {
        ...
        images.push('/path/to/field-image.jpg');
    ...
    focusPlayer: function($el) {
        ...
        $terrain.velocity("stop").velocity({
            opacity: 0.66
        }, {
            duration: 600,
            easing: 'easeOutQuad'
        ...
        this.showPlayerCard($el, 600, 200);
    },
    unfocusPlayer: function() {
        ...
        $(`.js-player`).not('.active').each(function() {
            const $unfocus = $(this);
            anim.fadeInDir($unfocus, 300, 0, 0, 0, 0, null, 1);
        });
    ...
    showPlayerCard: function($el, dur, delay) {
        ...
        anim.fadeOutDir($el.find('.player__label'), 200, 0);
        anim.fadeInDir($card, dur, delay, 0, 100, 0, 'easeOutQuad', 1);
    },
    hidePlayerCard: function($el, dur, delay) {
        ...
        if (isEmpty) {
            ...
            const playerId = Array.from(state.assignedPlayers.entries()).find(
                ([pId, slotIndex]) => slotIndex === $el.data('index')
            )?.[0];
            const $playerItem = playerId ? $(`.player-item[data-player-id="${playerId}"]`) : null;
            labelText = $playerItem
                ? $playerItem.data('name')
                : ($el.data('role-detail') || $el.data('role') || 'Unknown');
        }

        $el.find('.player__label span').text(labelText);

        anim.fadeInDir($el.find('.player__label'), 200, delay, 0, 0, 0, 'easeOutQuad');
        anim.fadeOutDir($card, dur, 0, 0, -100, 0, 'easeOutQuad', 0);
    },
